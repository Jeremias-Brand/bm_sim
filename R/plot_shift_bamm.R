...
all_mcmc_out_path <- paste0("out/bamm/", run, "_", rates,".trait_", priors, "_mcmc_out.txt")

basic_tmp <- lapply(all_mcmc_out_path, basic_stats_wrapper, expectedNumberOfShifts, burnin)
mcmc_stats_df <- rbindlist(basic_tmp) 

write.table("out/bamm/basic_stats_mcmc_out.txt", row.names = FALSE)
...
for (sim in sims){
  ...
  for (r in rates){
    res <- parLapply(cl = cl, run, get_mcmc_prob, r, prior_par)
    all_list <- c(all_list, res)
    }
  all_df <- rbindlist(all_list)
  
  pdf(paste0("out/plots/bamm_pre_post_sim", c, "_", prior_par, ".pdf"), width = 14, height = 14)
  ggplot(all_df) +
